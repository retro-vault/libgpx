...
void gpx_draw_line(gpx_t *g, coord x0, coord y0, coord x1, coord y1)
{
    ...
    { /* vertical line */
        ...
        if (y0 < g->clip_area.y0)
            y0 = g->clip_area.y0;
        if (y1 > g->clip_area.y1)
            y1 = g->clip_area.y1;
        /* Finally, quick draw. */
        _vline(x0, y0, y1);
    }
    ...
    { /* horizontal line */
        ...
        x1 = line_rect.x1;
        /* limit line to the inside of clip area */
        if (x0 < g->clip_area.x0)
            x0 = g->clip_area.x0;
        if (x1 > g->clip_area.x1)
            x1 = g->clip_area.x1;
        /* Finally, quick draw. */
        _hline(x0, x1, y0);
    }
...
void gpx_draw_polygon(gpx_t *g, int num_points, point_t points[])
{
    if (num_points < 2)
        return; /* no polygon */
    /* draw lines */
    ...
    {
        gpx_draw_line(g, points[i - 1].x, points[i - 1].y,
                      points[i].x, points[i].y);
    }
    /* last point back to first point */
    gpx_draw_line(g, points[num_points - 1].x, points[num_points - 1].y,
...
void gpx_fill_circle(gpx_t *g, coord x0, coord y0, coord radius) {
    ...
    /* note: calibrate line brush to y0-radius (top point)
       offset from (y0-radius) % brush size is index
       and number of shifts left for brush is (x0-left) % 8 */
    coord top = y0 - radius;
    coord left = x0 - radius;
    uint8_t ls = 0;

    gpx_set_line_style(g, (g->fill_brush)[(y0 - top) % (g->fill_brush_size)]);
    gpx_draw_line(g, x0 - radius, y0, x0 + radius, y0);

	while (x < y) {
        ls = _roleft(
            (g->fill_brush)[(y0 + x - top) % (g->fill_brush_size)],
            (x0 - y - left) % 8);
        gpx_set_line_style(g, ls);
        gpx_draw_line(g, x0 - y, y0 + x, x0 + y, y0 + x);
        ls = _roleft(
            (g->fill_brush)[(y0 - x - top) % (g->fill_brush_size)],
            (x0 - y - left) % 8);
        gpx_set_line_style(g, ls);
        ...
        {
            ls = _roleft(
                (g->fill_brush)[(y0 - y - top) % (g->fill_brush_size)],
                (x0 - x - left) % 8);
            gpx_set_line_style(g, ls);
            gpx_draw_line(g, x0 - x, y0 - y, x0 + x, y0 - y);
