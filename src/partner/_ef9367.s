...
__ef9367_set_line_style:
        ...
        ;;   AF
__ef9367_set_pen:
        or      a                       ; normalize 0/up, nonzero/down
        jr      z,.spen_cache
        ld      a,#1
.spen_cache:
        ...
        xor     #EF9367_CMD_PEN_UP      ; 0 -> pen up (3), 1 -> down (2)
...
.scolor_have:
        ...
        and     #1
        xor     #EF9367_CMD_DMOD_CLR    ; fore -> select pen, back -> eraser
        call    __ef9367_exec_cmd
        ld      a,#1                    ; both modes need the pen down
        jp      __ef9367_set_pen
...
.sxy_wait:
        ...
        ;; Both display heights are multiples of 256, so max_y has low
        ;; byte FF. Its low-byte subtraction never borrows into the high.
        ld      a,e
        cpl                             ; low byte of max_y - y
        out     (#EF9367_YPOS_LO),a
        ld      a,(__ef9367_max_y+1)
        sub     d
...
__ef9367_get_delta_cmd:
        ...
        rlca
        rlca
        and     #0x02                   ; command bit 1 = dx sign
        ld      b,a
        ld      a,d
        or      e
        jr      z,.gdc_ignore_dy
        ...
        rlca
        rlca
        and     #0x04                   ; command bit 2 = dy sign
        or      b
        xor     #0x15                   ; oblique command, reversed GDP Y
        ret
...
.gdc_ignore_dx:
        ...
        rlca
        rlca
        and     #0x02                   ; mirror dy sign into both directions
        ld      b,a
        rlca
        or      b
        xor     #0x14                   ; vertical command, reversed GDP Y
        ret
