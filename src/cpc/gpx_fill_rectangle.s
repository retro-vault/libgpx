...
_gpx_fill_rectangle::
        ...
        ;; -14..-15 fpatt ptr
        ;; -16      fpatt len
        ;; -17..-24 span descriptor for __gpx_span_row:
        ;;          -24 mask_first, -23 mask_last, -22 count,
        ;;          -21 sel_or,     -20 sel_xor,   -19 start parity,
        ;;          -18/-17 scratch used by the mode 1 run
        ;; -25      x phase shift (x0 original & 7)
        ld      hl,#-25
        add     hl,sp
        ld      sp,hl
        ...
        ld      a,-6(ix)
        ld      -12(ix),a
        ;; The pattern is anchored to the rectangle's own x0 and repeats
        ;; every eight pixels, so one rotation by (x0 & 7) puts it on the
        ;; pixel grid for the whole rectangle. In mode 1 those eight pixels
        ;; are two screen bytes; __gpx_span_row splits them.
        ld      a,-1(ix)
        and     #0x07
        ld      -25(ix),a
...
.fr_phase_setup:
        ...
        ld      l,-1(ix)                ; HL = x0 (clamped to the screen)
        ld      h,-2(ix)
        ld      e,-3(ix)                ; DE = x1
        ld      d,-4(ix)
        ld      b,4(ix)                 ; color
