...
OUT_DIR = ROOT / "bin" / "partner-visuals"
RAW_DIR = OUT_DIR / "raw"
HARNESS = ROOT / "build" / "tests" / "export_partner_visuals"
SCALE = 1
...
def partner_pixel(raw: bytes, width: int, x: int, y: int) -> int:
    stride = width // 8
    off = y * stride + (x >> 3)
    bit = 0x80 >> (x & 7)
...
def raw_to_pixels(raw: bytes, width: int, height: int):
    expected_size = (width // 8) * height
    ...
    for y in range(height):
        ...
        for x in range(width):
            out[base + x] = partner_pixel(raw, width, x, y)
    return width, height, out


def stack_pixels(top, bottom, w, h):
    stacked = [0] * (w * (h * 2))
    stacked[0 : w * h] = top
...
def write_case_txt(name: str, values: dict):
    ...
    height = values.get("height", "256")

    lines = [
        name,
        "",
        "Expected: top image (Partner backend) equals bottom image (oracle/expected).",
...
def main():
    ...
    failed = 0
    lines = [
        "# Partner Visual Compare",
        "",
        "Per test: stacked image with top=Partner backend, bottom=oracle/expected.",
        "",
    ]
    ...
    for meta_path in meta_files:
        ...
        height = int(values.get("height", "256"))
        ...
        if real_ok and oracle_ok:
            ...
            if real_raw.exists() and oracle_raw.exists():
                top = raw_to_pixels(real_raw.read_bytes(), width, height)[2]
                bottom = raw_to_pixels(oracle_raw.read_bytes(), width, height)[2]
                w, h, stacked = stack_pixels(top, bottom, width, height)
    ...
    print(f"generated {len(meta_files)} partner compare artifacts under {OUT_DIR}")
