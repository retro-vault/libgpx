...
Demo 5 is a single program, `polygons.c`, that exercises the two remaining
**advanced** primitives: `gpx_draw_polygon` and `gpx_fill_polygon`. Like
Demo 3 and Demo 4 it derives its layout from `gpx_width()` and
`gpx_height()`, so the same source fills a 256x192 ZX Spectrum display, a
1024x256 Iskra Delta Partner display, and either of the Amstrad CPC's two
modes.

It needs a library built with `ADVANCED` enabled, which is the default.
Built against an `ADVANCED=0` library the link fails with an unresolved
`_gpx_fill_polygon`.

The shapes are held in a 0..64 unit square and scaled into cells, so
adding one is a table entry rather than a screenful of coordinates.

## Polygons

The screen is three rows of four:

1. **Outlines** — a triangle, a hexagon in a long dash, a ten-point star,
   and a pentagram in a dotted line.
2. **Fills** — a convex hexagon; a concave chevron, whose notch has to
   stay empty; a pentagram, where the even-odd rule leaves the middle
   hollow; and a star filled with an eight-byte pattern.
3. **What the fill guarantees** — a dithered fill closed with a solid rim,
   a pentagram knocked out of a solid star, two triangles XOR'd so their
   overlap comes back out, and a chevron clipped to a window.

### ZX Spectrum 48K

![Polygons on the ZX Spectrum](../../docs/images/screenshots/polygons-zx.png)

### Iskra Delta Partner

![Polygons on the Iskra Delta Partner](../../docs/images/screenshots/polygons-partner.png)

### Amstrad CPC — 640 x 200

![Polygons on the Amstrad CPC at 640x200](../../docs/images/screenshots/polygons-cpc-640x200.png)

### Amstrad CPC — 320 x 200

![Polygons on the Amstrad CPC at 320x200](../../docs/images/screenshots/polygons-cpc-320x200.png)

As in Demo 4 the CPC pictures are stretched: a CPC pixel is not square, and
the captures are doubled vertically the way CPC screenshots conventionally
are. Nothing here is per machine — both polygon primitives are one
portable module in `src/common` driving each backend through
`gpx_draw_line` — so the four pictures are the same drawing at four
geometries. `tests/conformance` checks that phase by phase.

### What the rows show

**Row 1, the hexagon and the pentagram.** The dash pattern is carried from
each edge into the next, so it runs on round the corners instead of
restarting at every vertex, exactly as it does around a rectangle.

**Row 2, the chevron and the pentagram.** Concave shapes and
self-intersecting ones both work, because the fill pairs crossings on each
scanline by the even-odd rule rather than assuming a convex outline. The
pentagram's middle is genuinely outside the shape under that rule, which is
why it stays hollow — and why the same five points drawn as a ten-point
star in row 1 fill solid.

**Row 3, the dithered hexagon.** A `0xAA, 0x55` dither leaves the edge
ragged, because a pattern bit that is 0 is skipped rather than drawn.
Drawing the outline of the *same* points over it closes the edge exactly.
That the two agree is the fiddly part of the implementation: an edge the
path runs up has to be walked down to fill it, and this Bresenham is not
symmetric under reversal, so a flipped edge is walked with the opposite
rounding bias.

**Row 3, the XOR pair.** Two overlapping triangles in `BM_XOR`; the
overlap cancels because every row of a fill is painted exactly once.

**Row 3, the clipped chevron.** Fill and outline are both cut to the
dotted window, and the pattern keeps the phase it would have had
unclipped, so the mesh lines up with the window edge rather than
restarting at it.

### A note on stack

`gpx_fill_polygon` keeps its edge table on the stack — about 250 bytes
while the call runs, 17 per point. This demo's `crt0` therefore asks for
768 bytes rather than the 512 the other demos use. A program that fills
polygons on a tight stack should do the same.
...
```bash
make -C samples/demo5 build       # both targets
make -C samples/demo5 zx          # bin/demo5/polygons.tap
make -C samples/demo5 partner     # bin/demo5/polygons.com
```

All compilation happens in the pinned Docker toolchain images. Spectrum
tapes load at `0x8000`; Partner programs are CP/M transient programs at
`0x0100`.

## Run

Load the `.tap` in a 48K Spectrum emulator, or run the `.com` from CP/M on
the Partner:

```bash
scripts/run/run-fuse-demo1.sh bin/demo5/polygons.tap
```

To rebuild every sample screenshot, this one included:
